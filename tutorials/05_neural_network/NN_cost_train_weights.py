...
class NeuralNetwork(object):
    def __init__(self, lr = 0.01):
        ...
        with open("./weights/w1.p", "rb") as file:
            ...
            """
        self.w1 = np.zeros((10,100)) #Modell muss sie leernen
        self.lr = lr
    ...
    def train(self, X, y):
        a0 = self.activation(self.w0 @ X.T) #Ausgabe aus Layer1 --> Eingabe für folgendes
        #a0 hat shape (100, 60000) --> 100 Knoten in der Mitte == hidden layers
        pred = self.activation(self.w1 @ a0) #Ausgabe vom Layer1
        e = y.T - pred #error
        #UPDATE Regel
        #Gewichte fuer 2. Layer
        dw1 = e * pred *(1-pred) @ a0.T / len(X)#(10,60000) * (60000, 100) = (10, 100)
        #len(X) --> damit die Werte nicht zu groß werden

        #Update
        assert dw1.shape == self.w1.shape #Sicherstellen, dass keine zus. Dims kommen
        self.w1 = self.w1 + self.lr * dw1
        #Wenn Kosten runter gehen, dann lernt das Netz
